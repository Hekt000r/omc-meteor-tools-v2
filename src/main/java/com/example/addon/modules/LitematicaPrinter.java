...
public class LitematicaPrinter extends Module {
        ...
        if (timer >= printing_delay.get()) {
            BlockIterator.register(printing_range.get() + 1, printing_range.get() + 1, (pos, blockState) -> {
                ...
                if (
                    ...
                    boolean isBlockInLineOfSight = Utils.isBlockInLineOfSight(pos, required);
    ...
    public boolean place(BlockState required, BlockPos pos) {
        ...
        return Utils.place(pos, placeSide, wantedSlabType, wantedBlockHalf, wantedHorizontalOrientation != null ? wantedHorizontalOrientation : wantedHopperOrientation, wantedAxies, airPlace.get(), swing.get(), rotate.get(), clientSide.get(), printing_range.get());
    }

    private boolean switchItem(Item item, BlockState state, Supplier<Boolean> action) {
        if (mc.player == null) return false;

        int selectedSlot = ((PlayerInventoryAccessor)mc.player.getInventory()).getSelectedSlot();
        ...
        if (
            ...
            if (action.get()) {
                usedSlot = ((PlayerInventoryAccessor)mc.player.getInventory()).getSelectedSlot();
                return true;
            } else return false;
            ...
            Objects.requireNonNull(mc.getNetworkHandler()).sendPacket(new CreativeInventoryActionC2SPacket(36 + slot, item.getDefaultStack()));
    ...
    public enum SortAlgorithm {
        ...
        Nearest(false, Comparator.comparingDouble(value -> MeteorClient.mc.player != null ? meteordevelopment.meteorclient.utils.Utils.squaredDistance(MeteorClient.mc.player.getX(), MeteorClient.mc.player.getY(), MeteorClient.mc.player.getZ(), value.getX() + 0.5, value.getY() + 0.5, value.getZ() + 0.5) : 0)),
        Furthest(false, Comparator.comparingDouble(value -> MeteorClient.mc.player != null ? (meteordevelopment.meteorclient.utils.Utils.squaredDistance(MeteorClient.mc.player.getX(), MeteorClient.mc.player.getY(), MeteorClient.mc.player.getZ(), value.getX() + 0.5, value.getY() + 0.5, value.getZ() + 0.5)) * -1 : 0));
